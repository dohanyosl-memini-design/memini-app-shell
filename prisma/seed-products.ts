...
const products = [
  ...
  { sku: 'BLAUBEUREN-11', name: '(AQ) Blautopf a malommal (miénk) – színes faszelethez és kulcstartóhoz', nameDE: '(AQ) Blautopf mit Hammerschmiede (unsere) - zum bunte Holzscheibe und Schlüsselanhänger', city: 'Blaubeuren' },
...
async function main() {
  ...
  for (const p of products) {
    const result = await prisma.product.upsert({
      where: { sku: p.sku },
      update: { name: p.name, nameDE: p.nameDE ?? undefined, city: p.city },
      create: {
        sku: p.sku,
        name: p.name,
    ...
    if (result) {
      // Check if it was an update or create by seeing if createdAt ≈ now
      const age = Date.now() - new Date(result.createdAt as Date).getTime()
      if (age < 5000) created++
      else updated++
    }
  }
