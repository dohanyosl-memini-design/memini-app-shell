...
export default function DeliveryNoteForm({ onSave, onCancel, deliveryNote }: { onSave: () => void; onCancel: () => void; deliveryNote?: ExistingDeliveryNote | null }) {
  ...
  const [form, setForm] = useState({
    noteNumber: deliveryNote?.number || '',
    contactId: deliveryNote?.contact?.id || deliveryNote?.contactId || '',
    companyId: deliveryNote?.company?.id || deliveryNote?.companyId || '',
    date: deliveryNote?.date?.slice(0, 10) || today,
    deliveryInfo: deliveryNote?.deliveryInfo || '',
    currency: deliveryNote?.currency || 'EUR',
    notes: deliveryNote?.notes || '',
    billingName: deliveryNote?.billingName || '',
    billingAddress: deliveryNote?.billingAddress || '',
    billingZip: deliveryNote?.billingZip || '',
    billingCity: deliveryNote?.billingCity || '',
    billingCountry: deliveryNote?.billingCountry || '',
  })

  const [items, setItems] = useState<Item[]>(
    deliveryNote?.items?.length
      ? deliveryNote.items.map(i => i.isDiscount
  ...
  useEffect(() => {
    Promise.all([
      ...
      setAllContacts(c); setCompanies(co); setProducts(p); setCarriers(ca); setPricelist(pl)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const apiItems = items.map(item => {
      if (item.isDiscount) {
        ...
        return {
          description: d.discountType === 'percent' ? `- ${d.discountValue}% ${d.description}` : `- ${d.description}`,
          quantity: 1,
          unitPrice: -amount,
          vatRate: 0,
          isDiscount: true,
    ...
    const url = deliveryNote ? `/api/delivery-notes/${deliveryNote.id}` : '/api/delivery-notes'
    const method = deliveryNote ? 'PUT' : 'POST'
    const res = await fetch(url, {
      method,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ...form, number: form.noteNumber, items: apiItems }),
    })

    if (!res.ok) {
      const err = await res.json()
      alert(err.error || 'Hiba történt a szállítólevél kiállítása során.')
      setLoading(false)
      return
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="flex items-center gap-3 p-3 bg-green-50 border border-green-200 rounded-lg">
        <span className="text-sm font-semibold text-green-700 whitespace-nowrap">Szállítólevél szám:</span>
        <input
          type="text"
          value={form.noteNumber}
          onChange={e => setForm({ ...form, noteNumber: e.target.value })}
          placeholder="pl. SL-01/2026"
          required
          className="flex-1 px-3 py-1.5 border border-green-300 rounded-lg text-sm font-mono font-bold text-green-900 focus:outline-none focus:ring-2 focus:ring-green-500 bg-white"
        />
        <span className="text-xs text-green-500 whitespace-nowrap">szerkeszthető</span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        {form.companyId && (
          <div className="col-span-2 bg-white rounded-lg p-3 space-y-2 border border-gray-200">
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Szállítási cím</p>
                  ...
                  <tr key={idx}>
                    <td className="px-3 py-2">
                      ...
                      <input type="text" value={li.description}
                        ...
                        placeholder="Szállítólevelén megjelenő szöveg"
            ...
            {items.filter(i => i.isDiscount).map((item, i) => {
              ...
              return (
                <div key={i} className="flex justify-between text-red-500 text-xs">
                  <span>{d.discountType === 'percent' ? `−${d.discountValue}% ` : '−'}{d.description}:</span>
      ...
      <div>
        ...
        <textarea rows={2} value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })}
          placeholder="pl. Teillieferung – weitere Lieferung folgt"
      ...
      <div className="flex justify-end gap-3 pt-2">
        ...
        <button type="submit" disabled={loading}
          ...
          {loading ? 'Kiállítás...' : 'Szállítólevél kiállítása'}
        </button>
      </div>
    </form>
  )
