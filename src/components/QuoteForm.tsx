...
/** Az ajánlat érvényességére felkínált gyorsgombok. */
const VALIDITY_PRESETS = [14, 30, 60]
...
export default function QuoteForm({ onSave, onCancel, quote }: { onSave: () => void; onCancel: () => void; quote?: ExistingQuote | null }) {
  ...
  const valid30 = format(addDays(new Date(), 30), 'yyyy-MM-dd')

  const [form, setForm] = useState({
    quoteNumber: quote?.number || '',
    contactId: quote?.contact?.id || quote?.contactId || '',
    companyId: quote?.company?.id || quote?.companyId || '',
    date: quote?.date?.slice(0, 10) || today,
    validUntil: quote?.validUntil?.slice(0, 10) || valid30,
    currency: quote?.currency || 'EUR',
    notes: quote?.notes || '',
    billingName: quote?.billingName || '',
    billingAddress: quote?.billingAddress || '',
    billingZip: quote?.billingZip || '',
  ...
  const [items, setItems] = useState<Item[]>(
    quote?.items?.length
      ? quote.items.map(i => i.isDiscount
  ...
  function updateProduct(idx: number, field: keyof LineItem, value: string | number) {
    ...
    // A mennyiség változásakor az egységár átugrik a megfelelő árlista-sávba.
  ...
  // Az áfa a tételek tényleges kulcsaiból jön, nem fix 19%-ból.
  const vatAmount = items
    .filter(i => !i.isDiscount)
    .reduce((s, i) => {
      const li = i as LineItem
      return s + li.quantity * li.unitPrice * (li.vatRate / 100)
    }, 0)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const apiItems = items.map(item => {
      if (item.isDiscount) {
        ...
        // A készlet-feliratok már tartalmazzák a százalékot ("10% Rabatt"),
        // ezért csak akkor írjuk ki külön, ha még nem szerepel benne.
        const label = d.description.trim()
        const showPercent = d.discountType === 'percent' && !label.startsWith(`${d.discountValue}%`)
        return {
          description: showPercent ? `- ${d.discountValue}% ${label}` : `- ${label}`,
          quantity: 1,
          unitPrice: -amount,
          vatRate: 0,
          isDiscount: true,
    ...
    const res = await fetch(url, {
      ...
      body: JSON.stringify({ ...form, number: form.quoteNumber, items: apiItems }),
    })

    if (!res.ok) {
      const err = await res.json().catch(() => ({}))
      alert(err.error || 'Hiba történt az ajánlat mentése során.')
      setLoading(false)
      return
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="flex items-center gap-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
        <span className="text-sm font-semibold text-blue-700 whitespace-nowrap">Ajánlatszám:</span>
        <input
          type="text"
          value={form.quoteNumber}
          onChange={e => setForm({ ...form, quoteNumber: e.target.value })}
          placeholder="pl. AJ-2026-001"
          required
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        {/* Címzett — szerkeszthető */}
        {form.companyId && (
          <div className="col-span-2 bg-white rounded-lg p-3 space-y-2 border border-gray-200">
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Címzett az ajánlaton</p>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Ajánlat dátuma</label>
          <input type="date" value={form.date} onChange={e => setForm({ ...form, date: e.target.value })}
            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Érvényes eddig</label>
          <div className="flex gap-1.5 mb-1.5">
            {VALIDITY_PRESETS.map(days => (
              <button
                key={days}
                type="button"
                onClick={() => setForm({ ...form, validUntil: format(addDays(new Date(form.date || new Date()), days), 'yyyy-MM-dd') })}
                  ...
                  <tr key={idx}>
                    <td className="px-3 py-2">
                      ...
                      <input type="text" value={li.description}
                        ...
                        placeholder="Ajánlaton megjelenő szöveg"
      ...
      <div>
        ...
        <textarea rows={2} value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })}
          placeholder="pl. Lieferzeit ca. 2 Wochen."
      ...
      <div className="flex justify-end gap-3 pt-2">
        ...
        <button type="submit" disabled={loading}
          ...
          {loading ? 'Mentés...' : quote ? 'Ajánlat mentése' : 'Ajánlat létrehozása'}
        </button>
      </div>
    </form>
  )
