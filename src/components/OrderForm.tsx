...
import { Plus, X, AlertTriangle, TrendingDown, ChevronDown, ChevronUp, History } from 'lucide-react'
...
export default function OrderForm({ order, defaultCompanyId, onSave, onCancel }: OrderFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true)
    const body = {
      status, notes, internalNotes, customerRef, deliveryAddress, deliveryDate,
      date, contactId: contactId || null, companyId: companyId || null,
      ...
      {companyId && rankedProducts.length > 0 && (
        <div>
          ...
          <div className="flex flex-wrap gap-2 max-h-36 overflow-y-auto p-1">
            {rankedProducts.slice(0, 30).map(p => {
              const isPrev = prevProductIds.has(p.id)
              const isCity = selectedCompany?.city && p.city === selectedCompany.city
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border text-xs font-medium transition-all ${
                    alreadyAdded ? 'opacity-40 cursor-not-allowed border-gray-200 bg-gray-50 text-gray-400' :
                    isPrev ? 'border-blue-300 bg-blue-50 text-blue-700 hover:bg-blue-100' :
                    isCity ? 'border-green-300 bg-green-50 text-green-700 hover:bg-green-100' :
                    'border-gray-200 bg-white text-gray-600 hover:bg-gray-50'
                  }`}
                >
                  {isPrev && <span title="Korábban rendelve">⭐</span>}
                  {!isPrev && isCity && <span title="Városhoz illő">📍</span>}
                  <span>{p.name}</span>
                  {p.stock <= 5 && p.stock > 0 && <span className="text-orange-500 ml-1">({p.stock}db)</span>}
                  {p.stock === 0 && <span className="text-red-500 ml-1">(0db!)</span>}
                </button>
              )
            })}
          </div>
          <p className="text-xs text-gray-400 mt-1.5">⭐ = korábban rendelt · 📍 = városhoz illő</p>
        </div>
      )}
